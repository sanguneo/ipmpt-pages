...
function toSummary(p){ const imgs = p.images || []; const cover = imgs[0] || null; return { id:p.id, title:p.title||'', source:p.source||'', nsfw:!!p.nsfw, tags:p.tags||[], text:(p.text||'').slice(0,140), imageCount:imgs.length, cover: cover ? { id:cover.id, thumbUrl:cover.thumbUrl || cover.displayUrl || cover.imageUrl, imageUrl:cover.displayUrl || cover.imageUrl, width:cover.width||null, height:cover.height||null } : null, createdAt:p.createdAt, updatedAt:p.updatedAt }; }
...
function render(){
  ...
  for (const p of list){
    const img = p.cover; const label = esc(p.title || p.text.slice(0,60) || '무제'); const count = p.imageCount > 1 ? `<span class="countMark">${p.imageCount}</span>` : ''; const nsfw = p.nsfw ? '<b class="nsfwMark">NSFW</b>' : ''; const ratio = img?.width && img?.height ? img.height / img.width : 1; const ar = img?.width && img?.height ? ` style="aspect-ratio:${img.width} / ${img.height}"` : ''; const body = img ? `<img src="${esc(img.thumbUrl || img.imageUrl)}"${ar} loading="lazy" alt="">` : `<div class="ph">이미지 없음</div>`;
    let i = 0; for (let k = 1; k < n; k++) if (heights[k] < heights[i]) i = k; heights[i] += colW * ratio + 37 + gap;
    cols[i].push(`<article class="tile${p.nsfw ? ' nsfwTile' : ''}${img ? '' : ' emptyTile'}" data-open="${p.id}"><button class="tileBtn" type="button">${body}${nsfw}</button><div class="tileMeta"><span class="t">${label}</span>${count}</div></article>`);
  }
...
const showNsfw = document.querySelector('#showNsfw'); showNsfw.checked = localStorage.getItem('ipmpt.showNsfw') === '1'; document.body.classList.toggle('showNsfw', showNsfw.checked); showNsfw.addEventListener('change', () => { document.body.classList.toggle('showNsfw', showNsfw.checked); localStorage.setItem('ipmpt.showNsfw', showNsfw.checked ? '1' : '0'); });
